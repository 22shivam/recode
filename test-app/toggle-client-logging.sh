...
# Check current state by looking for logError
if grep -q "logError" app/page.tsx; then
  echo "🔴 Disabling client-side error logging..."

  # Create version WITHOUT error logging
...
EOF
  ...
  echo "✅ Client-side error logging DISABLED"
  echo "   ⚠️  Agent won't detect ArgumentValidationError bugs!"
  ...
  echo "🟢 Enabling client-side error logging..."

  # Run the add-error-logging script
  ./add-error-logging.sh
fi

echo ""
echo "💡 Current state:"
if grep -q "import { wrapMutation }" convex/todos.ts 2>/dev/null; then
  echo "   🟢 Server tracking: ON"
else
  echo "   🔴 Server tracking: OFF"
fi
